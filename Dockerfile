...
COPY --from=formatui /opt/formatui/dist/index.min.css public/css/format.min.css
COPY --from=formatui /opt/formatui/dist/index.min.js public/js/format.min.js
COPY --from=formatui /opt/formatui/src/img/formatui.svg public/img/format.svg
...
RUN python3 -m compileall -b ./internal/api/format
RUN rsync -av --include="*/" --include="*.pyc" --exclude="*" \
    internal/api/format ./lib
